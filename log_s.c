...
//Author: Nate Cowley
//borrowed heavily from previous deliverable
...
//this is where the magic happens. It's where the logServer actually writes the message to the logFile
void logEntry(char *message) 
{
  ...
  strftime(messageBuffer, timestampBufferSize, "%Y-%m-%d %H:%M:%S", timeStruct);//this formats the date and time
...
int main(int argc, char *argv[]) 
{
  ...
  // Install an interrupt handler to catch when user presses Ctrl+C (interrupt) to terminate the program.
  ...
  //continually create children to handle log requests
  ...
  while (1) 
  {
    ...
    addressLength = sizeof(cli_addr);
    numBytes = (int)recvfrom(udpSockFD, buffer, BUFFER_SIZE - 1, 0, (struct sockaddr *)&cli_addr, &addressLength);
	...
	//start child process and handling
    pid_t pid = fork();
    if(pid > 0) 
	{
      // This is the parent process. Do nothing here and keep looping in order not to block the server.
      continue;
    } 
	else if(pid == 0) 
	{
      ...
      return 0;  // Make sure to terminate the child process here in order to prevent too many children from existing
    } 
	else 
	{
