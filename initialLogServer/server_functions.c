...
void logMessage(int socketFD, char *buffer)
{
	struct sockaddr_in address;
	//       sendto sends a message to the (socket, message, messageLength, flag, sockaddr_in struct, sockaddr_in size
	int numBytes = (int)sendto(socketFD, buffer, strlen(buffer), 0, address, addressLength);
...
void processUDPRequest(int socketFD, struct sockaddr *address) {
  ...
  //copy the contents of confirmationMessage to buffer
...
void createLogServer()
{
	system("./logServer 9999");
}
