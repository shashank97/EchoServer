...
int main(int argc, char *argv[]) {
  ...
  //this is in server_functions, it uses a system call to activate the log server and sends it the address 9999
  createLogServer();
...
//this is an attempt to connect to the log server
int logSockFD = socket(AF_INET, SOCK_DGRAM, 0);

if (logSockFD < 0)
{
	error("ERROR opening socket for log server");
}
